...
if (TRUE == TRUE) {
  ...
  na_summary <- check %>%
    ...
    # Filter to show only variables with missing data (Optional - remove filter to see all)
    # filter(`Missing Count` > 0) %>%
    arrange(desc(`Missing Count`)) # Sort by most missing
...
tab_num_comp %>%
  kbl(
    caption = "Comparison of quantitative variables (Mean and SD).",
...
Healthcare expenditure data is characterized by two distinct statistical
hurdles: a skewness in positive costs (a few patients incur massive costs) and a
structural mass of zeros (many patients seek no care at all). In this dataset,
`r format(n_zeros, big.mark=",")` individuals (`r round(pct_zero, 1)`%) incurred
zero costs.
...
m_gamma_final <- glm(
  dvexpend ~ age + gender + general + mental + region + education_cat_detailed,
...
```{r plot-residuals, fig.height=5.4, fig.height = 6, fig.pos='!ht', fig.cap = "Idk bro I just want to sleep."}
par(mfrow=c(2,2))
plot(m_gamma_final)
```
...
```{r plot-residuals2, fig.height=5.4, fig.height = 6, fig.pos='!ht', fig.cap = "Idk bro I just want to sleep."}
par(mfrow=c(2,2))
plot(m_nb)
```
...
```{r from-individual-draft, echo=FALSE, include=FALSE}
# #################### MODELLING
# 1. Define Y as the Binary Outcome (0 or 1)
y_binary <- meps_clean$has_expense

# 2. Define X (Model Matrix using the FULL dataset, not just positive)
X_full <- model.matrix(
  has_expense ~ . -
    dvisit -
    dvexpend -
...
# 3. Fit Lasso with family = "binomial" (Logistic)
set.seed(123)
...
X_pos <- model.matrix(
  dvexpend ~ . -
    dvisit -
    dvexpend -
    ndvisit - # not assumed to affect once you are in doctor's office
    ndvexpend -
    has_expense -
    education -
    education_cat,
...
##############################
# Actual models
# Part 1: Probit Model for Probability of Any Expense
# We use the FULL dataset (meps_clean) because we are predicting Yes/No for everyone.
...
```{r coef-table-two-part, results='asis', message=FALSE, warning=FALSE}
# --- Helper Function ---
...
clean_data <- raw_data %>%
  mutate(
    ...
    var_group = case_when(
      ...
      str_starts(term, "income")    ~ "Income (USD)",
      str_starts(term, "mental")    ~ "Mental Health (ref: Excellent)",
      str_starts(term, "ndvisit")   ~ "Number of non-doctor visits",
      str_starts(term, "region")    ~ "Region (ref: Northeast)",
      TRUE ~ "Other"
    ),
    
    # B. Define the Row Labels (stripping prefixes)
    label = case_when(
      term == "(Intercept)" ~ "(Intercept)",
    ...
    # C. CUSTOM ORDERING LOGIC (The "Fix")
    
    # Create a ranking variable for the specific rows you want ordered logically
    # Lower number = appears first in the group
    label_rank = case_when(
      ...
      # Health Order (Poor -> VGood)
      label == "Poor"  ~ 1,
      label == "Fair"  ~ 2,
      label == "Good"  ~ 3,
      label == "VGood" ~ 4,
      
      # Default for everything else (alphabetical sorts equal ranks)
      TRUE ~ 10 
    )
  ) 

# --- 3. Enforce the Group Order ----------------------------------------------

# Identify all unique groups in the data
all_groups <- unique(clean_data$var_group)

# Define the specific top priority groups
top_groups <- c(
  "Intercept", 
  "General Health (ref: Exc)", 
  "Mental Health (ref: Exc)"
)

# Combine: Top groups + the rest (sorted alphabetically)
final_group_order <- c(
  top_groups, 
...
sorted_data <- clean_data %>%
  # 1. Turn var_group into a factor with our custom order
  mutate(var_group = factor(var_group, levels = final_group_order)) %>%
  # 2. Arrange by Group, then by our Custom Rank, then Alphabetically
...
# Extract indices for grouping
group_indices <- sorted_data %>%
  mutate(row = row_number()) %>%
  group_by(var_group) %>%
  summarise(start = min(row), end = max(row)) %>%
  arrange(start) # Ensure indices follow the table order

# Select final columns (exclude helper columns)
...
caption_text <- paste0(
  ...
  beta_symbol,
  "), standard errors, and p-values for the two-part MEPS expenditure model. A Probit and Gamma GLM (log link) were used in combination to model the probability of incurring any expense (pt 1: Probit) and the expenditure among health care users for patients who incurred a healthcare expenditure (pt 2: Gamma GLM). Variable selection was performed independently for each step."
)
...
# Apply Grouping Headers
for(i in 1:nrow(group_indices)) {
  group_name <- as.character(group_indices$var_group[i])
  # Skip Intercept header if you want (optional)
  if(group_name == "Intercept") next 
...
```{r coeftableexpendaturetwopart, results='asis', message=FALSE, warning=FALSE}
# --- Helper Function ---
format_p_vec <- function(x) {
  x_num <- as.numeric(x)
  out <- rep("", length(x_num))
...
clean_data <- raw_data %>%
  mutate(
    ...
    var_group = case_when(
      ...
      # Grouped together to be sorted, but header will be hidden
      ...
      # --- Categorical Variables (With explicit reference headers) ---
      str_starts(term, "education") ~ "Education (ref: Less than High School)",
      str_starts(term, "ethnicity") ~ "Ethnicity (ref: White)",
      str_starts(term, "gender")    ~ "Gender (ref: Female)",
      str_starts(term, "general")   ~ "General Health (ref: Excellent)", 
    ...
    label = case_when(
      ...
      # Categorical Labels (strip variable name prefix)
      str_starts(term, "education_cat_detailed") ~ str_remove(term, "education_cat_detailed"),
      str_starts(term, "ethnicity") ~ str_remove(term, "ethnicity"),
      str_starts(term, "gender")    ~ str_remove(term, "gender"),
      str_starts(term, "general")   ~ str_remove(term, "general"),
    ...
    # C. CUSTOM ORDERING LOGIC for Rows
    label_rank = case_when(
      # Education Order
      label == "High School Graduate" ~ 1,
      label == "Some College"         ~ 2,
...
# UPDATED: Must match the new text strings exactly for sorting to work
top_groups <- c(
  "Intercept", 
  "Non_Categorical", 
...
for(i in 1:nrow(group_indices)) {
  ...
  # Skip headers for Intercept and the Non_Categorical group
...
\begin{align}
% Part 1: Probit
\text{Part 1: } \Pr(\text{expend}_i > 0) &= \Phi(\eta_{1i}) \nonumber \\
...
&\quad + \gamma_7 \text{GeneralHealth}_i + \gamma_8 \text{MentalHealth}_i \nonumber \\
&\quad + \gamma_9 \text{Hypertension}_i + \gamma_{10} \text{Hyperlipidemia}_i \nonumber \\
&\quad + \gamma_{11} \text{Income}_i + \gamma_{12} \text{NdVisit}_i \\[10pt]
% Part 2: Gamma
\text{Part 2: } \ln(\mu_i) &= \beta_0 + \beta_1 \text{Age}_i + \beta_2 \text{Gender}_i \nonumber \\
&\quad + \beta_3 \text{Region}_i + \beta_4 \text{Education}_i \nonumber \\
&\quad + \beta_5 \text{GeneralHealth}_i + \beta_6 \text{MentalHealth}_i \nonumber \\
&\quad + \beta_7 \text{Hypertension}_i + \beta_8 \text{Hyperlipidemia}_i \nonumber \\
&\quad + \beta_9 \text{Income}_i + \beta_{10} \text{NdVisit}_i
\end{align}


<!-- Φ(⋅): The Cumulative Distribution Function (CDF) of the standard normal distribution (standard notation for Probit models). -->

<!-- η1i​: The linear predictor for the probability stage. -->

<!-- μi​: The expected expenditure conditional on it being positive (E[Yi​∣Yi​>0]). -->

<!-- γ vs β: It is important to use different Greek letters to emphasize that the effect of a variable (e.g., Age) on the probability of spending (γ1​) is different from its effect on the amount of spending (β1​). -->

<!-- Note that $\text{BMI}_{i}$ and $\text{Ethnicity}_{i}$ appear in the first  -->
<!-- equation but are absent in the second, based on the Lasso variable selection. -->
...
```{r tmp-utilisation, echo = FALSE}
mean_visits <- mean(meps_clean$dvisit)
var_visits <- var(meps_clean$dvisit)
```

To analyse the frequency of physician encounters (`r
format_var_name("dvisit")`), a Negative Binomial regression model was employed.
...
Poisson model. The Negative Binomial distribution accounts for this unobserved
heterogeneity by including an additional dispersion parameter (θ)."
