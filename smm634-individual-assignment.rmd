...
higher costs than those in "Excellent" health.  

These patterns are consistent with clinical expectations: older, sicker, and
more health-engaged individuals both enter the system more often and incur
higher costs once they do so.

The two-part model achieves an Root Mean Square Error of
$`r round(rmse, 0)` and a Mean Absolute Error of $`r round(mae, 0)`. 
...
```{r expenboxplot, echo=FALSE, fig.cap="test"}
# boxplot
...
Rate Ratios < 1, Table \@ref(tab:coeftablecount)), consistent with structural
barriers to access documented in the US healthcare system [@cook2010].
Non-physician visits (`r format_var_name("ndvisit")`) were positively 
...
more doctor's appointments also tend to have higher-than-expected costs per
visit, suggesting a compounding resource burden for patients with higher health
care demands.

As illustrated in Figure \@ref(fig:plots-copula), there is a clear positive
...
indicates a dependence that extends beyond simple accumulation. This suggests
that 'heavy users' of the healthcare system are distinct not only in their visit
frequency but also in their resource consumption intensity, supporting the use
of a joint frequency-severity modelling framework.


```{r plots-copula, echo=FALSE, fig.cap="Joint distribution of healthcare utilisation and expenditure for patients with at least one doctor's visit. The vertical banding reflects the discrete nature of visit counts and the y-axis shows total doctor-visit expenditure (dvexpend) on a logarithmic scale to accommodate skewness. The overlaying positive trend highlights that expenditure increases with visit frequency. The model-estimated Kendall’s $\tau$ quantifies the residual dependence, confirming that frequency and severity are positively correlated even after adjusting for covariates.", fig.align='center', out.width="80%"}
...
This analysis highlights the complex drivers of healthcare demand. The two-part expenditure model confirms that the decision to seek care and the resulting cost intensity are driven by overlapping but distinct magnitudes of effect. The significant gender and health-status gaps reinforce the need for risk-adjustment models that explicitly account for biological and systemic usage differences [@bertakis2000].
...
doctor-visit expenditure. 

However, the error distribution is highly asymmetric:
