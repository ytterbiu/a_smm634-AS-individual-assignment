...
div(style = "background-color: #f8f9fa; padding: 20px; border: 1px solid #e9ecef; border-radius: 5px; text-align: center; margin-bottom: 30px;",
  ...
  p("This static report is accompanied by a live R Shiny dashboard."),
  a(href = "https://3enji.shinyapps.io/smm634-meps-dashboard/", 
    target = "_blank",
    class = "btn btn-primary", 
    style = "background-color: #007bff; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px; font-weight: bold;",
    "Launch interactive dashboard to explore the models described below!")
)
```
...
observations on US adults aged 18–65 years. Tables \@ref(tab:initialOverviewCat) &
...
and `r format_var_name("education")`), socio-economic status 
(`r format_var_name("income")`),
health indicators (`r format_var_name("BMI")`, self-reported 
`r format_var_name("general")` and `r format_var_name("mental")` health, 
...
tab_num_comp <- bind_rows(
  ...
  # expenditure
  get_num_compare(meps_clean, meps_pos, "dvexpend", "Doctor expenditure (USD)", digits=2, big_mark=",", show_mean_se = TRUE),
  get_num_compare(meps_clean, meps_pos, "ndvexpend", "Non-doctor expenditure (USD)", digits=2, big_mark=",", show_mean_se = TRUE)
)
...
This approach avoids the retransformation bias present in log-normal models
...
(`r format_var_name("ethnicity")`, `r format_var_name("region")`, 
`r format_var_name("education_cat_detailed")`
`r format_var_name("general")` and `r format_var_name("mental")` 
...
\log(\mu_i) &= \beta_0   + \beta_1 \text{age}_i   + \beta_2 \mathrm{I}(\text{gender}_i)   + \beta_3 \text{bmi}_i \nonumber \\
&\quad + \sum_{k=2}^{5} \beta_{4k} \mathrm{I}(\text{general}_i = k)   + \sum_{k=2}^{5} \beta_{5k} \mathrm{I}(\text{mental}_i = k) \nonumber \\
&\quad + \sum_{k=2}^{4} \beta_{6k} \mathrm{I}(\text{ethnicity}_i = k)   + \sum_{k=2}^{4} \beta_{7k} \mathrm{I}(\text{region}_i = k) \nonumber \\
&\quad + \beta_8 \mathrm{I}(\text{hypertension}_i)   + \beta_9 \mathrm{I}(\text{hyperlipidemia}_i) \nonumber \\
&\quad + \beta_{10} \text{income}_i   + \beta_{11} \text{ndvisit}_i \nonumber \\
&\quad + \sum_{k=2}^{5} \beta_{12k} \mathrm{I}(\text{education\_cat\_detailed}_i = k) (\#eq:utiliNegBin)
\end{align}
...
using the `GJRM` package in R [@MarraRadice2025CopulaADDR]. 
...
associated with a `r round(100*(exp(10*coef(m_part2)["age"])-1))`%
increase in conditional costs. Interestingly, males incur 
...
health-engaged both enter the system more often and incur higher costs once they do so.
...
(`r round(100*(exp(coef(m_count)["ndvisit"])-1))`% increase per
...
estimated Gaussian copula parameter (Kendall’s $\hat{\tau} =$ `r round(tau_hat,3)`, 
$\hat{\theta} =$ `r round(theta_hat,3)`)
...
data, and also found that 'heavy' patient's are distinct, not only in their visit
...
framework. The estimated Gaussian copula parameter ($\hat{\tau}$ = 
`r round(tau_hat,3)`) aligns with recent work by
@marramodelling2025 on the same MEPS dataset. @marramodelling2025, despite 
using a more advanced Rotated Copula with Gumbel and Dagum models, 
reported a similar dependence ($\hat{\theta}$ = 2.67 corresponding to $\hat{\tau}$ =
0.625^[Calculated using $\tau = 1 - \frac{1}{\theta}$ for the Gumbel
Kendall’s tau relationship.]). This report's replication of this finding using a
parsimonious Gamma-Gaussian framework further validates the link between visit
frequency and cost intensity as a fundamental structural feature of healthcare
demand, rather than a model artefact.

Interpreting these coefficients as causal effects is constrained by
observational biases. While the analysis identified ethnic disparities, chiming
...
to coverage types, potentially biasing socio-economic conclusions. The positive
...
dependence. Future work may address these catastrophic costs, potentially by
using tree-based machine learning to detect
...
residual plots in Figure \@ref(fig:diag-gamma) show minor deviations at 
the largest fitted values.
...
To complement the regression analysis, address space constraints, and in a 
optimistic pursuit of potential extra credit, an interactive R Shiny web
application was developed to allow one to explore the models described in this 
report further. This dashboard, or web application, allows one to
dynamically input specific patient characteristics and observe the resulting
predictions from the fitted models. It is accessible via: 

- https://3enji.shinyapps.io/smm634-meps-dashboard/

```{r appendixdashboard1, echo=FALSE, purl=FALSE, out.width="100%", fig.cap="Annotated interface of the MEPS Healthcare Model Dashboard. The interface highlights (1) a button that runs the model for the selected parameters, (2) patient covariate configuration, (3) results navigation, and (4) the predicted expenditure and utilisation outcomes."}
knitr::include_graphics("fig/appendix-dashboard-1.pdf")
```

Figure \@ref(fig:appendixdashboard1) shows an annotated view of the dashboard, 
including:

1. An Estimate Outcomes button, which runs the prediction logic using the 
selected inputs against using the Two-Part and Negative Binomial models.
1. A sidebar containing interactive sliders and input fields for key covariates, 
including continuous and categorical variables.
1. Two tabs allowing the user to toggle between the numerical estimates and a 
visual comparison of the patient against the population distribution.
1. Key results cards displaying the calculated Expected Annual Expenditure 
(derived from the combined Probit and Gamma models) and the Expected Doctor 
Visits (derived from the Negative Binomial model).
...
# Reproducibility, accessibility, & word count
...
This report was created in R Markdown. The source code is open source and
available via:
...
`r word_count <- (241+78+204+218+55+195+62+139+362)`
...
`r word_count` (`r sprintf("%+.1f%%", 100 * (word_count - 1500) / 1500)`).
